...
int main() {
  ...
  RecordMode::Record record_ = RecordMode::Record(
    //fmt::format("{}{}", CONFIG_FILE_PATH, "/record/recordpath_save.yaml"),
        fmt::format("{}{}", CONFIG_FILE_PATH, "/record/record_packeg/"),
  ...
  //vw_src.open(CONFIG_FILE_PATH + save_path_ + to_string(Record::getFileSizeInOrder(save_path_)) + ".avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 66, cv::Size(1280, 800), true);
  ...
  while (true) {
    ...
    if (record_.mv_capture_.isindustryimgInput()) {
      src_img_ = record_.mv_capture_.image();
    } else {
      record_.capture>>src_img_;
    }
    record_.Run(src_img_);
    record_.mv_capture_.cameraReleasebuff();
    //double t = (double(cv::getTickCount() - t1)) / cv::getTickFrequency();
    std::clock_t end = std::clock();
    double   t = (double)(end - start) / CLOCKS_PER_SEC;
    sum += t;
    if (sum >= 5) {
      record_.close(save_path_, src_img_);
      sum = sum - 5;
    }
    if (!src_img_.empty()) {
      serial_.updateReceiveInformation();

      switch (uart::PLANE_MODE) {  //serial_.returnReceiveMode()
      // 基础自瞄模式
      case uart::SUP_SHOOT:
    ...
    if (global_fps_.returnFps() > 500) {
      record_.mv_capture_.~VideoCapture();
