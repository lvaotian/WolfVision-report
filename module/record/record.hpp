...
class Record {
 public:
  Record();
  Record(std::string path_in, cv::Size size);
  ~Record();
  /**
   * @brief  图像录制函数
  ...
  */
  ...
  void Run(cv::Mat cap);
  int  getFileSizeInOrder(const std::string& path);
  void close(std::string path_in, cv::Mat src_img);
};
}  // namespace RecordMode
